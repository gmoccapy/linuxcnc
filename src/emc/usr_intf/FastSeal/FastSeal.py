...
DATADIR = os.path.join( BASE, "share", "FastSeal" )
IMAGEDIR = os.path.join( DATADIR, "images" )
XMLNAME = os.path.join( DATADIR, "FastSeal.glade" )
...
# the ICONS should must be in share/FastSeal/images
...
class FastSeal( object ):
    def __init__( self ):
        ...
        locale.bindtextdomain( "FastSeal", LOCALEDIR )
        gettext.install( "FastSeal", localedir = LOCALEDIR, unicode = True )
        gettext.bindtextdomain( "FastSeal", LOCALEDIR )

        # needed components to comunicate with hal and linuxcnc
        self.halcomp = hal.component( "FastSeal" )
        ...
        self.builder.set_translation_domain( "FastSeal" )
...
#        self.rapidrate = 1         # holds the rapid override value and is needed to be able to react to halui pin
        ...
        self.widgets.window1.set_title( "EU Surplus FastSeal %s" % _RELEASE )
