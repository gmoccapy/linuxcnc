...
class Touchpad():
    ...
    def enter(self):
        try:
            ...
            if self.dro.get_text() == "" or self.dro.get_text() == "-":
                ...
                text = self.dro.get_text()
                # ToDo : Evaluate the text and set the new value, before sending activate
                #        Otherwise it has to be done on every widget in the main program

                #         May be it is even more convenient to make an own entry widget with included touchpad 
    ...
    def show(self, widget, units='in', position=None):
        ...
        print("Type of widget = ", type(widget))
        # gtk.SpinButton   need to find adjustment to modify that values
        # gtk.Entry        can work with text only
